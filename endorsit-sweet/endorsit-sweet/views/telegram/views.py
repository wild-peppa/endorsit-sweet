...
def airdrop(token):
    ...
    reply(
        '钱包地址提交成功，您获得的EDS将在活动结束后 15 个工作日内发放。 \nThe NEO address is submitted successfully, and the EDS you get will be issued within 15 working days after airdrop ended.\n\n*******************\n\n邀请 %(invite)s 人\n获得 %(earned)s EDS\n\nInvite %(invite)s\nEarned %(earned)s EDS' % {
            'invite': str(validator.invited_count),
            'earned': str(validator.earned)
        })
    return make_response('true')
